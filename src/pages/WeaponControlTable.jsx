...
import { Equipment, WeaponControlItem, Soldier, ActualStorageCount } from "@/entities/all";
...
export default function WeaponControlTable() {
  ...
  const [weaponControlItems, setWeaponControlItems] = useState([]);
  ...
  const pageTranslations = {
    en: {
      title: "Weapon Control Table",
      subtitle: "Weapon status summary by platoons",
      refreshData: "Refresh Data",
      tryAgain: "Try Again",
      weaponInventory: "Weapon Inventory Summary",
      filterPlatoons: "Filter Platoons",
      downloadCsv: "Download CSV"
    },
    he: {
      title: "תבלת שליטה נשק",
      subtitle: "סיכום מצב נשקים לפי פלוגות",
      refreshData: "רענן נתונים",
      tryAgain: "נסה שוב",
      weaponInventory: "סיכום מלאי נשקים",
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const weaponItemsData = await WeaponControlItem.list();
      await new Promise(resolve => setTimeout(resolve, 500)); // Added artificial delay for Soldier data
  ...
  const processedData = useMemo(() => {
    if (equipment.length === 0 || weaponControlItems.length === 0 || selectedPlatoons.length === 0 || soldiers.length === 0) {
      return { isEmpty: true };
    }

    const equipmentTypes = weaponControlItems.map(item => String(item.name).trim()).filter(Boolean);
  ...
  }, [equipment, weaponControlItems, selectedPlatoons, soldiers, language]); // Added 'language' to dependencies for UNASSIGNED_SQUAD_LABEL
  ...
  const handleDownloadCSV = () => {
    ...
    if (processedData.isSinglePlatoonView) {
      ...
      if (isDetailedView) {
        ...
        const footer = ["Total", platoonTotal.total, platoonTotal.issued, platoonTotal.storage, platoonTotal.repair];
        csvRows.push(toCsvRow(footer));
      }
    } else {
      // Multi-platoon view remains the same
      const { platoons, equipmentTypes, tableData, totalsByPlatoon, grandTotal } = processedData;
      const csvHeader = ["Equipment", ...platoons.flatMap(p => currentSubHeaders.map(sub => `${p} - ${sub}`)), ...currentSubHeaders.map(sub => `Grand Total - ${sub}`)];
      csvRows.push(toCsvRow(csvHeader));
      ...
      <div className="p-4 md:p-6 lg:p-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={handleRefresh} disabled={isLoading || isRateLimited}>
        ...
        <Card className="shadow-lg">
          <CardHeader>
            <CardTitle>{pt.weaponInventory}</CardTitle>
          </CardHeader>
