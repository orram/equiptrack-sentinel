      ...
      <Table className="min-w-full divide-y divide-slate-200">
        <TableHeader>
          <TableRow className="bg-slate-50">
            ...
            <TableHead colSpan={subHeaders.length} className={`text-center font-bold text-white bg-slate-800 ${isHe ? 'sticky left-0 z-10 border-r-2 border-slate-400' : 'border-r-2 border-slate-400'}`}>
          ...
          <TableRow className="bg-slate-50">
            ...
            {subHeaders.map((sub, index) => (
              <TableHead key={`grand-total-${sub}`} className={`text-center text-xs font-semibold text-slate-300 bg-slate-800 border-l ${isHe ? 'sticky left-0 z-10' : ''} ${index === 0 ? 'border-l-2 border-slate-400' : ''}`}>
                {sub}
              </TableHead>
            ))}
          </TableRow>
        ...
        <TableBody>
          {equipmentTypes.map(type => (
            <TableRow key={type} className="hover:bg-slate-50">
              ...
              {subHeaders.map((sub, index) => (
                <TableCell key={`${type}-grand-total-${sub}`} className={`text-center font-semibold bg-slate-100 border-l ${isHe ? 'sticky left-0 z-10' : ''} ${index === 0 ? 'border-l-2 border-slate-400' : ''}`}>
        ...
        <TableFooter>
          <TableRow className="bg-slate-200 font-bold text-slate-900 border-t-2 border-slate-400">
            ...
            {subHeaders.map((sub, index) => (
              <TableCell key={`total-grand-total-${sub}`} className={`text-center bg-slate-900 text-white border-l ${isHe ? 'sticky left-0 z-10' : ''} ${index === 0 ? 'border-l-2 border-slate-400' : ''}`}>
